...
object NaClLowLevel {
  ...
  private val _9: ByteArray = ByteArray(32).apply {
    this[0] = 9
  }

  private val gf0: LongArray = LongArray(16) {
    0
  }

  private val gf1: LongArray = LongArray(16).apply {
    this[0] = 1
  }
  ...
  private fun randomBytes(x: ByteArray, size: Int) {
    require(x.size >= size) {
      "array must be of size>=`$size` but it is of size=${x.size}"
    }
  ...
  private fun dl64(x: ByteArray, xi: Int): Long {
    require(x.size >= 8 + xi) {
      "Array must have at least 8 elements for `Byte`s to `Long` conversion"
    }
  ...
  private fun st32(x: ByteArray, off: Int = 0, u: Int) {
    require(x.size >= 4 + off) {
      "`x` output array is too small to fit 4 bytes starting from $off"
    }
...
//    ).asLongArray()
  ...
  private fun crypto_sign(sm: ByteArray, m: ByteArray, n: Long, sk: ByteArray): Int {
    require(sm.size >= n + 64) {
      "resulting array sm(size=${sm.size}) must be able to fit n+64 bytes (${n + 64})"
    }
